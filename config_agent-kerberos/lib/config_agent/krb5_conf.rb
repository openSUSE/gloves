...
module ConfigAgent
  class Krb5Conf < ConfigAgent::FileAgent
    ...
    def get(params)
      ...
      # FIXME use exception!
      unless aug.get("/augeas/files#{KERBEROS_CONFIG}/error").nil?
        aug.close
        return {}
      end

      default_realm	= aug.get("/files#{KERBEROS_CONFIG}/libdefaults/default_realm")
      default_realm	||= ""
      ...
      clockskew	= aug.get("/files#{KERBEROS_CONFIG}/libdefaults/clockskew")
      krb5_conf["clockskew"]	= clockskew unless clockskew.nil?

      realms = aug.match("/files/#{KERBEROS_CONFIG}/realms/realm[*]")
      ...
      aug.match("/files#{KERBEROS_CONFIG}/appdefaults/application/*").each do |sub_path|
    ...
    def put(params)
      ...
      aug.set("/files#{KERBEROS_CONFIG}/libdefaults/default_realm", default_realm) if default_realm
      aug.set("/files#{KERBEROS_CONFIG}/libdefaults/clockskew", params["clockskew"]) if params["clockskew"]

      # update existing realm section
      realm_save_path		= ""

      realms		= aug.match("/files#{KERBEROS_CONFIG}/realms/*")
      ...
      if realm_save_path.empty? && default_realm
        realm_save_path	= "/files#{KERBEROS_CONFIG}/realms/realm[#{realms.size + 1}]"
      ...
      if default_realm && default_domain
        aug.set("/files#{KERBEROS_CONFIG}/domain_realm/." + default_domain, default_realm)
      end
      ...
      unless aug.save
        ret["success"]	= false
        ret["message"]	= aug.get("/augeas/files#{KERBEROS_CONFIG}/error/message")
      end
  ...
  private
    ...
    def write_trusted_servers aug, params
      ...
      appdefaults	= aug.match("/files#{KERBEROS_CONFIG}/appdefaults/*")
      pkinit_exists = appdefaults.any? {|sub_path| aug.get(sub_path) == "pkinit" }
      pkinit_path	= "/files#{KERBEROS_CONFIG}/appdefaults/application"
      unless pkinit_exists
        # create new subsection
        pkinit_path	= "/files#{KERBEROS_CONFIG}/appdefaults/application[#{appdefaults.size + 1}]"
    ...
    def write_pam aug, params
      ...
      pam_path	= "/files#{KERBEROS_CONFIG}/appdefaults/application"
      ...
      ["ticket_lifetime", "renew_lifetime", "forwardable", "proxiable", "minimum_uid",
       ...
       "subsequent_prompt", "banner"].each do |pam_key|
        if params.has_key? pam_key
          unless pam_exists
            pam_path	= "/files#{KERBEROS_CONFIG}/appdefaults/application[#{appdefaults.size + 1}]"
