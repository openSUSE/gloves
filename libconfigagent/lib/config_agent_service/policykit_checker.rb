...
module ConfigAgentService
  # Provides functionality to check permission for given Dbus sender
  module PolicykitChecker
    # Ensures that user have permission otherwise raise exception
    # @param sender of dbus message
    # @param [String] permission to check
    # @param [Hash] options to modify behavior
    # @option options [True] "only_noninteractive_permission_check" (False) set if policy kit should skip interaction from user ( explicit password ). If set and permission require retype of password, that behave like user doesn't have permission.
