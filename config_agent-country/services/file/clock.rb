...
class Clock < ConfigAgentService::FileService
  ...
  def write(params)
    ...
    aug.transform(:lens => "Shellvars.lns", :incl => "/etc/sysconfig/clock")
    aug.load
    ret        = {
      "success"	=> true
    }

    path        = "/files/etc/sysconfig/clock/"
    ...
    unless aug.save
      ret["success"]	= false
      ret["message"]	= aug.get("/augeas/files/etc/sysconfig/clock/error/message")
    end
